    ...
    for fname in ["hypertension_data.csv", "hypertension_dataset.csv"]:
        ...
        if os.path.isfile(path):
            try:
                ...
                col_maps = {
                    "BP_History":     {"normal": 0, "elevated": 1, "high": 2, "prehypertension": 1, "hypertension": 2},
...
def _build_hypertension_features(p: dict, activity_int: int) -> np.ndarray:
    ...
    if model is not None:
        try:
            n_expected = getattr(model, "n_features_in_", 8)
        except AttributeError:
            pass

    if n_expected == 10:
        # Construct the 10-feature vector
        salt_val = 12.0 if salt_int == 2 else (8.0 if salt_int == 1 else 5.0)
        stress_val = 8 if stress_int == 2 else (5 if stress_int == 1 else 2)
        ...
        sleep_dur = 7.0
        med_ft = 0.0
        smoking_status = 2.0 if p.get("smoking") else 0.0
        
        return np.array([[
            age, salt_val, stress_val, bp_hist, sleep_dur, bmi,
            med_ft, family_history, float(activity_int), smoking_status
        ]])
    else:
        # Fallback to the 8-feature synthetic structure
        return np.array([[
            age, systolic_bp, diastolic_bp, bmi,
            family_history, float(activity_int), float(salt_int), float(stress_int)
        ]])
