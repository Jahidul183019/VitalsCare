...
def explain_risk(disease: str, risk_data: Dict[str, Any], patient: dict, lang: str = "en") -> str:
    ...
    if not factors or factors[0].get("input") == "none":
        if lang == "bn":
            level_bn_map = {"low": "কম", "medium": "মাঝারি", "high": "উচ্চ"}
            level_bn = level_bn_map.get(level.lower(), level)
            return f"আপনার {disease_name} এর ঝুঁকি {prob:.1f}% ({level_bn} ঝুঁকি)। কোনো জটিল ক্ষতিকারক কারণ পাওয়া যায়নি।"
        return f"Your risk for {disease_name} is {prob:.1f}% ({level} risk). No major risk factors were detected."
