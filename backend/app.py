...
async def assess(patient: PatientData):
    try:
        ...
        if patient_dict.get("bmi", 0) <= 0:
            raise HTTPException(status_code=400, detail="Invalid BMI")
        if patient_dict.get("systolic_bp", 0) <= 0 or patient_dict.get("diastolic_bp", 0) <= 0:
            raise HTTPException(status_code=400, detail="Invalid blood pressure")
...
async def about():
    ...
    try:
        models_data = models_info.get('models', {})
        parts = []
        for key, name in [('diabetes', 'Diabetes'), ('malnutrition', 'Malnutrition'), ('hypertension', 'Hypertension'), ('cvd', 'CVD')]:
            acc = models_data.get(key, {}).get('accuracy')
            if acc is not None:
                parts.append(f"{name}: {acc * 100:.2f}%")
            else:
                parts.append(f"{name}: N/A")
        accuracy_str = ", ".join(parts)
    except Exception:
        accuracy_str = "Accuracy data unavailable"
