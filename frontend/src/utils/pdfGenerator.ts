...
export function downloadPDFReport(
  ...
  const tableRows = [
    ...
    { label: "Fasting Blood Sugar", val: String(data.fastingBloodSugar || "normal").toUpperCase(), range: "Normal" },
    { label: "Cholesterol Profile", val: String(data.cholesterol || "normal").toUpperCase(), range: "Normal" },
    { label: "Body Mass Index (BMI)", val: `${(data.weight / ((data.height / 100) * (data.height / 100))).toFixed(1)} kg/m²`, range: "18.5-24.9" },
  ...
  tableRows.forEach((row, index) => {
    ...
    const isAbnormalVal = row.val.includes("HIGH") || row.val.includes("ACTIVE") || 
                          (row.label.includes("Blood Pressure") && (data.systolic >= 140 || data.diastolic >= 90 || htnRisk >= 70)) ||
                          (row.label.includes("BMI") && ((data.weight / ((data.height / 100) * (data.height / 100))) >= 25 || dbRisk >= 70));
    if (isAbnormalVal) {
