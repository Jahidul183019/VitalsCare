...
interface ProfilePanelProps {
  ...
  onSignUp: (email: string, pass: string, name: string) => boolean | string;
  onSignOut: () => void;
}
...
export default function ProfilePanel({
  ...
  const handleAuthAction = (e: React.FormEvent) => {
    ...
    if (activeTab === "login") {
      const res = onLogin(emailText, passwordText);
      ...
      const res = onSignUp(emailText, passwordText, nameText);
      if (typeof res === "string") {
        setErrorMessage(res);
      } else if (res === true) {
